...
const AssetSubtype = require('../controller/subtype');
...
exports.onQuery = async function (request, response, next) {
    try {
        ...
        const doc = await AssetSubtype.onQuery(querys);
...
exports.onQuerys = async function (request, response, next) {
    try {
        ...
        const doc = await AssetSubtype.onQuerys(querys);
        var resData = await resMsg.onMessage_Response(0,20000)
        resData.data = doc;
        response.status(200).json(resData);
    } catch (err) {
...
exports.onCreate = async function (request, response, next) {
    try {
        if (Array.isArray(request.body)) {
            const doc = await AssetSubtype.onCreateMany(request.body);
            var resData = await resMsg.onMessage_Response(0,20000)
            resData.data = doc;
            response.status(200).json(resData);
        } else {
            const doc = await AssetSubtype.onCreate(request.body);
            var resData = await resMsg.onMessage_Response(0,20000)
            resData.data = doc;
            response.status(200).json(resData);
        }
...
exports.onUpdate = async function (request, response, next) {
    try {
        let resData;
        if (Array.isArray(request.body)) {
            const doc = await AssetSubtype.onUpdateMany(request.body);  
            resData = await resMsg.onMessage_Response(0, 20000);
            resData.data = doc;
        } else {
            const query = { _id: new mongo.ObjectId(request.body._id) };
            const doc = await AssetSubtype.onUpdate(query, request.body);
            resData = await resMsg.onMessage_Response(0, 20000);
            resData.data = doc;
        }
...
exports.onDelete = async function (request, response, next) {
    ...
    if (Array.isArray(request.body.id)) {
        const doc = await AssetSubtype.onDeleteMany(request.body.id);
        ...
        const doc = await AssetSubtype.onDelete(query);
        const resData = await resMsg.onMessage_Response(0, 20000);
            resData.data = doc;
                return response.status(200).json(resData);
    }
